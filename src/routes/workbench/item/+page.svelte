<script lang="ts">
	...
	import ItemInput from '$lib/workbench/item/ItemInput.svelte';
	...
	const clearData = () => {
		currentItem = { ...DefaultItem, researchKeys: [] };
		selected = '';
		showFeedback = false;
	};
	const loadData = () => {
		if (selected && data.itemLibrary[selected]) {
			// We only allow item entries to be selected
			const item: ItemData = data.itemLibrary[selected].item as ItemData;
			currentItem = { ...item, researchKeys: [...item.publications.map(pub => pub.key)] };
	...
	const downloadData = () => {
		generateAndDownloadItem(currentItem);
		notification?.show('success', 'Download ready!');
	};

	let currentItem: ItemDataHelper = $state({ ...DefaultItem, researchKeys: [] });
...
<ItemInput bind:item={currentItem} {showFeedback} researchLibrary={data.researchLibrary} />
