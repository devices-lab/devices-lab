...
<ClassBox {props} class="">
	<el-dropdown>
		...
		<el-menu id={menuId} anchor="bottom" popover="auto" class="origin-top-right overflow-visible rounded-md shadow-2xl ring-1 ring-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] backdrop:bg-transparent open:block focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
			{@render children()}
		</el-menu>
	</el-dropdown>
</ClassBox>
