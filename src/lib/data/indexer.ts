...
import { findRoute, type BreadcrumbLink, type Route } from '$lib/data/routes';
...
export const DefaultItem: ItemData = {
	name: '',
	pathName: '',
	teaser: '',
	description: '',
...
export function generateAndDownloadItem(item: ItemDataHelper): void {
	...
	const researchKeys = removeDups(item.researchKeys);
	...
	const replacer = (key: string, value: unknown) => {
		...
		if (key === "researchKeys") return undefined; // omit from final output
		return value ?? '';
	}
...
export const item: ItemData = ${JSON.stringify(item, replacer, 4)};
`;
