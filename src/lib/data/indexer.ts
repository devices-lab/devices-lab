...
import type { FamilyData, ItemData } from '$lib/data/data';
...
export type EntryItem = Entry & { kind: 'item'; item: ItemData };
export type EntryFamily = Entry & { kind: 'family'; item: FamilyData };

export type CatalogEntry = { data: EntryItem[] | EntryItem | undefined; type: 'family' | 'item' | 'none'; family: EntryFamily | undefined };
...
const build = () => {
	...
	const findEntries = (path: string): CatalogEntry => {
		...
		if (itemData && itemData.kind === 'item') {
			return { data: itemData as EntryItem, type: 'item', family: entries[itemData.parent] as EntryFamily | undefined };
		}

		// Check if we have a family for this path
		const listData = getChildren(path);
		if (listData && listData.length > 0) {
			return { data: listData as EntryItem[], type: 'family', family: entries[listData[0].parent] as EntryFamily | undefined };
		}

		// No match found
		return { data: undefined, type: 'none', family: undefined };
	};
...
export const DefaultItem: EntryItem = {
	kind: 'item',
	item: {
		modified: '0',
		name: '',
...
export const DefaultFamily: EntryFamily = {
	kind: 'family',
	item: {
		name: '',
		pathName: '',
