<script lang="ts">
	...
	const { type, role, origin, dx = 0, dy = 0, width, height, radius = 10, fill = 'none', borderColor = 'none', borderWidth = 2, topLeft = false, topRight = false, bottomLeft = false, bottomRight = false, border, ...restProps }: Props = $props();
...
{:else}
	<rect x={actualOrigin.x} y={actualOrigin.y} {width} {height} rx={border.radius} {...commonProps} />
{/if}
