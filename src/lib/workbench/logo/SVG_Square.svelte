<script lang="ts">
	...
	const maxRadius = $derived(Math.min(data.height / 2, data.width / 2));
...
<Preview {uid} bind:this={preview} dataString={dataString} bind:data={loadedData}>
	{#snippet config()}
		...
		<InputGroup label="Common">
			<NumberInput 	label="Width" 		bind:value={data.width}				initial={Defaults.width} 			min={0} max={500} />
			<NumberInput 	label="Height" 		bind:value={data.height}			initial={Defaults.height} 			min={0} max={500} />
		...
		<InputGroup label="Border">
			<NumberInput 	label="Radius" 		bind:value={data.border.radius} 	initial={Defaults.border.radius} 	min={0} max={maxRadius} />
			<NumberInput 	label="Width" 		bind:value={data.border.width} 		initial={Defaults.border.width} 	min={0} max={hasBorder ? Math.min(data.height, data.width) : 0} />
	...
	<SVG {uid} width={data.width + data.border.width} height={data.height + data.border.width} class={className}>
		<Rect type="path" role="subject" {origin} dx={0} dy={0} width={data.width} height={data.height} radius={data.border.radius} fill={data.fill} topLeft topRight bottomLeft bottomRight />
		<Text role="clip" {origin} dx={textX} dy={textY} text={data.text} fontSize={data.fontSize} width={textWidth} color={data.color} {...bold} />

		<!-- Border -->
		<Rect type="path" role="frame" {origin} width={data.width} height={data.height} radius={data.border.radius} borderColor={data.border.color} borderWidth={data.border.width} topLeft topRight bottomLeft bottomRight />
	</SVG>
</Preview>
