...
function dxfHeaderMM(minX: number, minY: number, maxX: number, maxY: number, layers: string[]) {
	const layerTable = layers.map((name, i) => ['0', 'LAYER', '2', name, '70', '0', '62', String(i ? 1 : 7), '6', 'CONTINUOUS'].join('\n')).join('\n');
	return [
		'0',
		'SECTION',
		'2',
...
function dxfLwPolyline(ring: Pt[], layer: string) {
	...
	for (let i = 0; i < ring.length; i++) {
		const a = ring[i];
		if (!pts.length || Math.hypot(a.x - pts[pts.length - 1].x, a.y - pts[pts.length - 1].y) > 1e-9) pts.push(a);
		if (i === ring.length - 1 && (a.x !== ring[0].x || a.y !== ring[0].y)) pts.push({ x: ring[0].x, y: ring[0].y });
	}
	const n = pts.length;
	const head = [
		'0',
...
export async function exportDxfForAltium(id: string, filename: string) {
	...
	if (!rings.length) rings = unionBlack(clone, 0.05);
	if (!rings.length) {
	...
	const outers: Pt[][] = [],
		holes: Pt[][] = [];
	for (let i = 0; i < tr.length; i++) (hole[i] ? holes : outers).push(tr[i]);
	outers.forEach(orientCCW);
	holes.forEach(orientCW);

	const header = dxfHeaderMM(ext.minX, ext.minY, ext.maxX, ext.maxY, ['OUTER', 'CUTOUT']);
	const body = [...outers.map((r) => dxfLwPolyline(r, 'OUTER')), ...holes.map((r) => dxfLwPolyline(r, 'CUTOUT'))].join('');
	const dxf = header + '\n' + body + dxfFooter();
...
function unionBlack(svg: SVGSVGElement, tol = FLATTEN_TOLERANCE): Path[] {
	...
	const polys = blacks
		.map(elToPathD)
		.filter(Boolean)
		.flatMap((d) => parseAndFlattenPath(d, tol));
...
function booleanBlackMinusWhite(svg: SVGSVGElement, tol = FLATTEN_TOLERANCE): Path[] {
	...
	const blackPolys = blacks
		.map(elToPathD)
		.filter(Boolean)
		.flatMap((d) => parseAndFlattenPath(d, tol));
	const whitePolys = whites
		.map(elToPathD)
		.filter(Boolean)
		.flatMap((d) => parseAndFlattenPath(d, tol));
	...
	if (uw.length) c.AddPaths(uw, ClipperLib.PolyType.ptClip, true);
	const out: Path[] = [];
	...
	const cleaned = rings
		.map((r) => {
			...
			for (let i = 0; i < r.length; i++) {
				const a = r[i];
				if (i === 0 || Math.hypot(a.x - out[out.length - 1].x, a.y - out[out.length - 1].y) > 1e-6) out.push(a);
				if (i === r.length - 1 && (out[0].x !== out[out.length - 1].x || out[0].y !== out[out.length - 1].y)) out.push({ x: out[0].x, y: out[0].y });
			}
			return out;
		})
		.filter((r) => r.length >= 4);
	...
	for (let i = 0; i < cleaned.length; i++) {
		if (hole[i]) orientCW(cleaned[i]);
		else orientCCW(cleaned[i]);
	}
	return cleaned;
}
...
function evenOddHoles(rings: Path[]): boolean[] {
	...
	return rings.map((r, i) => {
		const p = r[0];
		let c = 0;
		for (let k = 0; k < rings.length; k++) if (k !== i && inside(p, rings[k])) c++;
		return c % 2 === 1;
	});
}

function orientCCW(r: Path) {
	// positive area
	let A = 0;
	for (let i = 0; i < r.length - 1; i++) A += r[i].x * r[i + 1].y - r[i + 1].x * r[i].y;
	if (A < 0) r.reverse();
	return r;
}
...
function rectToCubicPathD(el: SVGRectElement) {
	...
	const cx = rx * KAPPA,
		...
		x1 = x + w,
		y1 = y + h;
	return [`M ${x + rx} ${y}`, `H ${x1 - rx}`, `C ${x1 - rx + cx} ${y} ${x1} ${y + ry - cy} ${x1} ${y + ry}`, `V ${y1 - ry}`, `C ${x1} ${y1 - ry + cy} ${x1 - rx + cx} ${y1} ${x1 - rx} ${y1}`, `H ${x + rx}`, `C ${x + rx - cx} ${y1} ${x} ${y1 - ry + cy} ${x} ${y1 - ry}`, `V ${y + ry}`, `C ${x} ${y + ry - cy} ${x + rx - cx} ${y} ${x + rx} ${y}`, `Z`].join(' ');
}
...
function circleOrEllipseToCubicPathD(el: SVGCircleElement | SVGEllipseElement) {
	...
	const ox = rx * KAPPA,
		oy = ry * KAPPA;
	return [`M ${cx + rx} ${cy}`, `C ${cx + rx} ${cy - oy} ${cx + ox} ${cy - ry} ${cx} ${cy - ry}`, `C ${cx - ox} ${cy - ry} ${cx - rx} ${cy - oy} ${cx - rx} ${cy}`, `C ${cx - rx} ${cy + oy} ${cx - ox} ${cy + ry} ${cx} ${cy + ry}`, `C ${cx + ox} ${cy + ry} ${cx + rx} ${cy + oy} ${cx + rx} ${cy}`, `Z`].join(' ');
}
...
function pointsToPathD(el: SVGPolygonElement | SVGPolylineElement) {
	...
	for (let i = 2; i < pts.length; i += 2) segs.push(`L ${pts[i]} ${pts[i + 1]}`);
	if (el.tagName.toLowerCase() === 'polygon') segs.push('Z');
	return segs.join(' ');
}

function elToPathD(el: Element): string {
	const t = el.tagName.toLowerCase();
	if (t === 'path') return (el as SVGPathElement).getAttribute('d') || '';
	if (t === 'rect') return rectToCubicPathD(el as SVGRectElement);
	if (t === 'circle' || t === 'ellipse') return circleOrEllipseToCubicPathD(el as SVGCircleElement | SVGEllipseElement);
	if (t === 'polygon' || t === 'polyline') return pointsToPathD(el as SVGPolygonElement | SVGPolylineElement);
	return '';
}
