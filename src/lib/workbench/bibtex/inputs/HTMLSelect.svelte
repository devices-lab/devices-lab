...
{#snippet SelectItem(label: string, sublabel: string, value: string)}
	...
	<el-option {value} class="group/option disabled relative block cursor-default py-2 pr-9 pl-3 text-gray-900 select-none focus:bg-primary-600 focus:text-white focus:outline-hidden dark:text-white dark:focus:bg-primary-500 disabled:text-red-300">
		<div class="flex">
...
<el-select {value} class="mt-2 block" {...props} bind:this={selectBox}>
	<button type="button" class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-2 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-primary-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus-visible:outline-primary-500">
