		...
		<div class="relative mx-auto flex items-center justify-between px-4 py-1 sm:px-6 sm:py-3 lg:px-8">
			...
			<div class="absolute top-15 right-5 flex gap-2 sm:top-20 sm:gap-4">
				{#each page.data.SideMenu as route}
					<IconLink href={route.id} icon={{ icon: route.icon, class: 'size-4 sm:size-5' }} tooltip={{ content: route.title }} class="sm:border-1 flex rounded-full border-1 border-gray-400 p-1.5 text-gray-400 shadow-md transition-transform duration-100 hover:scale-110 sm:p-2" />
				{/each}
			</div>
