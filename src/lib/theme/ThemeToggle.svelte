<script lang="ts">
	...
	import type { ThemeMode } from '$lib/theme/Toggle.svelte';
	...
	function applyTheme(mode: ThemeMode) {
		if (mode === 'light') applyClass(false);
		else if (mode === 'dark') applyClass(true);
		else applyClass(!!mql?.matches);
	}
	...
	function getStoredTheme(): ThemeMode | null {
		...
		return typeof raw === 'string' && (raw === 'light' || raw === 'dark' || raw === 'auto') ? raw : null;
	}

	function enableSystemSync(enable: boolean) {
		if (!mql) return;
		mql.onchange = null;
		if (enable) {
			mql.onchange = (e) => {
				if (localStorage.getItem(STORAGE_KEY) === 'auto') applyClass(e.matches);
			};
		}
	}

	onMount(() => {
		// set up mql and initial theme
		mql = window.matchMedia('(prefers-color-scheme: dark)');
		mode = getStoredTheme() ?? 'auto';
		applyTheme(mode);
		enableSystemSync(mode === 'auto');
	});
...
<div class="mt-2 flex justify-around border-t border-gray-200 dark:border-gray-700">
	...
	<Toggle bind:mode activeMode="auto" uid={`theme-toggle-${uid}`}>
		<svg viewBox="0 0 28 28" fill="none">
	...
	<Toggle bind:mode activeMode="dark" uid={`theme-toggle-${uid}`}>
		<svg viewBox="0 0 28 28" fill="none">
