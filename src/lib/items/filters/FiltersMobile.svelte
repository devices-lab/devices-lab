...
<el-dialog>
	<dialog id={id_mobileFilters} class="overflow-hidden backdrop:bg-transparent sm:hidden">
		...
		<div tabindex="-1" class="fixed inset-0 flex focus:outline-none">
			<el-dialog-panel class="relative ml-auto flex size-full max-w-xs transform flex-col overflow-y-auto bg-white pt-4 pb-6 shadow-xl transition duration-300 ease-in-out data-closed:translate-x-full">
				<div class="flex items-center justify-between px-4">
					...
					<button type="button" command="close" commandfor={id_mobileFilters} class="relative -mr-2 flex size-10 items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-50 focus:ring-2 focus:ring-primary-500 focus:outline-hidden">
