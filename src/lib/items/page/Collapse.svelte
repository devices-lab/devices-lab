<script lang="ts">
	import type { Snippet } from 'svelte';

	export interface CollapseItem {
		label: string;
		children: Snippet;
		[key: string]: any;
	}

	const { label, children, ...restProps }: CollapseItem = $props();
	const uid = $props.id();
</script>

<div>
	<h3>
		<button type="button" command="--toggle" commandfor={uid} class="group relative flex w-full items-center justify-between py-6 text-left">
			<span class="text-sm font-medium not-in-aria-expanded:text-gray-900 dark:not-in-aria-expanded:text-gray-300 in-aria-expanded:text-indigo-600 dark:in-aria-expanded:text-indigo-400">{label}</span>
			<span class="ml-6 flex cursor-pointer items-center">
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" data-slot="icon" aria-hidden="true" class="size-6 text-gray-400 group-hover:text-gray-500 in-aria-expanded:hidden">
					<path d="M12 4.5v15m7.5-7.5h-15" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" data-slot="icon" aria-hidden="true" class="size-6 text-indigo-400 not-in-aria-expanded:hidden group-hover:text-indigo-500">
					<path d="M5 12h14" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</span>
		</button>
	</h3>
	<el-disclosure id={uid} class="contents" {...restProps}>
		<div class="pb-6">
			{@render children()}
		</div>
	</el-disclosure>
</div>
